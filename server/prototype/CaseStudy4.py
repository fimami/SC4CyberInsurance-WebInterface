...
new_json_file_location = 'ContractNew.json'
new_json_hash = get_hash_of_file(new_json_file_location)
...
remove_contract_from_db_with_hash(insurer_connection, new_json_hash)
remove_contract_from_db_with_hash(customer_connection, new_json_hash)
...
message = requests.get('http://localhost:5001/proposeToUpdateContract/ContractNew.json').content.decode('UTF-8')
print(message)

message = requests.get('http://localhost:5000/useContract/' + json_hash).content.decode('UTF-8')
print(message)

message = requests.get('http://localhost:5000/agreeToUpdateContract').content.decode('UTF-8')
print(message)
...
contractBalanceAfter = round(w3.fromWei(w3.eth.getBalance(contract_address), 'ether'))
customerBalanceAfter = round(w3.fromWei(w3.eth.getBalance(customer), 'ether'))
#assertions not 100% correct as new premium is only around 1.85
#assert contractBalanceBefore == contractBalanceAfter - 2
#assert customerBalanceBefore == customerBalanceAfter + 2
